    ...
    public Coordinate getPlanetRebound(Coordinate coordinate) {
        try {
            for (int depth = 0; depth < size; depth++) {
                ...
                } else if (leftAbovePlanet.contains(new CoordinateDepthPair(coordinate, depth))) {
                    coordinate = coordinate.right(size + 2 * depth + 1).down(size + 2 * depth + 1);

                } else if (leftBelowPlanet.contains(new CoordinateDepthPair(coordinate, depth))) {
                    coordinate = coordinate.right(size + 2 * depth + 1).up(size + 2 * depth + 1);
